...
namespace wcge {
	namespace c3d {
		...
		public:
			...
			inline size_t CreateTexture(const std::wstring& filename, const bool bDitheringEnabled = true) { m_vpTextures.push_back( new CBImage(Image(filename), bDitheringEnabled) ); return m_vpTextures.size() - 1; }
			inline size_t CreateTexture( const uint32_t w, const uint32_t h ) { m_vpTextures.push_back(new CBImage(w, h) ); return m_vpTextures.size() - 1; }
			...
			inline CBImage& GetTexture( const size_t i ) { return *(m_vpTextures[i]); }
			...
			inline void BindVertexShaderTexture( const size_t nTextureId, const size_t nPosition ) { m_pBoundVertexShader->binding.pTextures[nPosition] = m_vpTextures[nTextureId]; }
			...
			inline void BindPixelShaderTexture( const size_t nTextureId, const size_t nPosition ) { m_pBoundPixelShader->binding.pTextures[nPosition] = m_vpTextures[nTextureId]; }
		...
		private:
			...
			std::vector<CBImage*> m_vpTextures;
